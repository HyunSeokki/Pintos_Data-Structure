...
{
	int i,j,s=byte_cnt(bitmap_size(bitmap));
	size_t size2;
	unsigned long *new = (unsigned long*)malloc(sizeof(unsigned long) * s);

	for(i=0; i<s; i++)
		new[i] = bitmap->bits[i];

	size2 = bitmap_size(bitmap) + size;
	j = byte_cnt(size2);
	bitmap->bits = realloc(bitmap->bits,j);
	bitmap_set_all(bitmap,0);
	bitmap->bit_cnt = size2;
